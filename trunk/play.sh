...
get_general_infos () {	      
		...
		MPLAYER_TEST=`mplayer -identify "$INPUT" -really-quiet -frames 0 -vo null  2>&1 /dev/nul | grep - -e "ID_VIDEO" -e "ID_AUDIO_" -e "ID_LENGTH" -e "ID_DEMUXER" `  
	...
	pal-dar133 (){
	 ...
	 pal-dar177(){
		...
		RATIO_I=`echo "($RATIO * 100) /1"|bc `
	      ...
	      if [[ ( $RATIO_I  -ge 123 && $RATIO_I -lt 127 ) && (-z $DAR || $DAR  == 1.25 ) ]]
	      then
# 		 mv $INPUT /home/fredo/tr-encoder/pal/p/${OUTPUT}.org
#  		 continue
		 
		 DETECTED_FORMAT="1.25"
		 echo -e "\\n${pink}Format: $DETECTED_FORMAT ( pal reencoded !)${NC}"
		 ...
		 echo -e "\\n${red}Format: $DETECTED_FORMAT${NC}"
	      ...
	      if [[ ( $RATIO_I  -gt 127 && $RATIO_I -lt 160 ) && (-z $DAR || $DAR == 1.33 ) ]]
	      then
		 ...
		 if [[ ( $RATIO_I  -ge 127 && $RATIO_I -lt 160 ) && ( $DAR  == 1.77 ) ]]
		 then
		...
		# get new sizes
		 calc_new_sizes
	      ...
	      if [[ $RATIO_I -ge  199 && $RATIO_I -lt 256 ]]
	      then
...
# 				fi 	    
		...
		LOGO_X=$(echo "(($NEW_WIDTH * 10) / 100) "|bc)
		LOGO_Y=$(echo "scale=3;(($NEW_HEIGHT  * 30) / 100) - $PADTOP "|bc)

		else
		# no padding 
		echo "scale=3;(($NEW_HEIGHT / 100 ) * 30 ) + $CROPTOP"
		LOGO_X=$(echo "scale=3;(($NEW_WIDTH  / 100) *  10) + $CROPLEFT"|bc)
		LOGO_Y=$(echo "scale=3;(($NEW_HEIGHT / 100 ) * 30 ) + $CROPTOP"|bc)
		fi
